...
export default async function ReviewPage({ params }: { params: { id: string } }) {
  ...
  // Signed URLs for document viewing (service role)
  ...
  return (
    <div className="space-y-6">
      <Link href="/admin" className="text-sm text-zinc-400 hover:text-zinc-200">← Back to queue</Link>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
              ...
              <div className="grid gap-2">
                {docs.map((d) => (
                  <div key={d.id} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-4 py-2.5 text-sm">
                    ...
                    {d.url ? (
                      <a href={d.url} target="_blank" rel="noreferrer" className="text-indigo-400 hover:underline">View / download</a>
        ...
        <div className="lg:col-span-1">
          <DecisionPanel
            ...
            status={app.status}
            principal={app.requested_amount ?? 0}
            tenure={app.tenure_months}
            existingRemark={app.admin_remark ?? ""}
          />
        </div>
      </div>
    </div>
