...
export async function AnalyticsView() {
  ...
  const { data: apps } = await supabase.from("applications").select("status, requested_amount").neq("status", "Draft");
  const { data: disb } = await supabase.from("disbursements").select("principal, total_payable");
...
function Kpi({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (<div className={`glass rounded-xl p-5 ${accent ? "ring-1 ring-accent/30" : ""}`}>
    ...
    <div className="mt-1 text-2xl font-bold">{value}</div></div>);
}
function Metric({ label, value }: { label: string; value: string }) {
  return (<div className="flex items-center justify-between border-b border-white/5 pb-2 text-sm">
    <span className="text-zinc-400">{label}</span><span className="font-semibold text-zinc-100">{value}</span></div>);
}
