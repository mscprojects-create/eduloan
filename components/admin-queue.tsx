...
type App = {
  ...
  university: string | null; requested_amount: number | null; status: string;
  created_at: string;
};

export function AdminQueue({ apps }: { apps: App[] }) {
        ...
        <div className="flex gap-1">
          {["All", ...STATUSES].map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              className={`rounded-lg px-3 py-1.5 text-xs transition-colors ${filter === s ? "bg-accent text-white" : "text-zinc-400 hover:bg-white/5"}`}>
              {s}
            </button>
          ))}
        </div>
        <button onClick={() => setSortDesc((v) => !v)} className="ml-auto text-xs text-zinc-400 hover:text-zinc-200">
          Date {sortDesc ? "↓" : "↑"}
        </button>
      </div>
          ...
          {rows.map((a) => (
            <tr key={a.id} className="border-b border-white/5 transition-colors hover:bg-white/[0.02]">
              ...
              <td className="px-4 py-3 text-right">
                <Link href={`/admin/applications/${a.id}`} className="text-indigo-400 hover:underline">Review</Link>
              </td>
            </tr>
          ))}
          {rows.length === 0 && (
            <tr><td colSpan={6} className="px-4 py-12 text-center text-zinc-500">No applications match.</td></tr>
          )}
        </tbody>
      </table>
    </div>
